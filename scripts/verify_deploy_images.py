...
이 스크립트가 막는 실수:
  - api와 scheduler가 서로 다른 커밋의 candidate 이미지를 가리키는 실수
    ...
    이어야 한다).
  - 배포하려는 candidate 이미지가 실제로 지금 배포하려는 git 커밋(HEAD 또는
    명시적으로 지정한 SHA)으로 빌드된 게 맞는지 확인 없이 그냥 진행하는 실수.
...
이 스크립트가 하지 않는 것:
  ...
  - web은 api/scheduler와 독립적으로 배포될 수 있으므로(정적 파일 서빙만
    하고 DB migration과 무관) web의 revision이 api/scheduler와 다르다고
    ERROR로 취급하지 않는다 - 참고용으로만 표시한다.
...
--expected-sha를 생략하면 이 저장소의 `git rev-parse HEAD`를 기준으로 삼는다.
...
def verify(api_image: str, scheduler_image: str, expected_sha: Optional[str]) -> list[str]:
    """검증 결과 ERROR 메시지 목록을 반환한다(비어 있으면 통과). web은 api/
    scheduler와 독립 배포 가능하므로 이 함수의 판정 대상이 아니다(호출부가
    참고용으로만 별도 출력한다)."""
    errors: list[str] = []
    ...
    if api_info.revision_sha and scheduler_info.revision_sha:
        ...
        elif expected_sha and api_info.revision_sha != expected_sha:
            errors.append(
                f"api/scheduler 이미지의 revision({api_info.revision_sha[:12]})이 "
                f"기대한 커밋({expected_sha[:12]})과 다릅니다."
            )
...
def _print_report(
    api_image: str, scheduler_image: str, web_image: Optional[str], expected_sha: Optional[str], errors: list[str]
) -> None:
    api_info = inspect_image(api_image)
    scheduler_info = inspect_image(scheduler_image)
    print("=== candidate 이미지 revision 검증 ===")
    print(f"기대 커밋(expected_sha): {expected_sha[:12] + '...' if expected_sha else '(미지정)'}")
    ...
    if web_image:
        ...
        print(f"web        : {web_image} (api/scheduler와 달라도 ERROR 아님 - 참고용)")
    ...
    if errors:
        ...
        print("\n[PASS] api/scheduler revision 일치, 배포 진행 가능.")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        expected_sha = args.expected_sha or git_head_sha()
        errors = verify(args.api_image, args.scheduler_image, expected_sha)
        _print_report(args.api_image, args.scheduler_image, args.web_image, expected_sha, errors)
